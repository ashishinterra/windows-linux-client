...
" /var/log/ccpyd.log"
        ...
        if [[ "${ccpy_state}" == "OK" ]]; then
            ...
            local installer_package=KeyTalkClient-${client_version}-${osspec}.tgz
            local installer_package_path="${REPO_DIR}/Software/Client/Projects/Export/${installer_package}"
            echo "Collecting installer package ${installer_package}"
