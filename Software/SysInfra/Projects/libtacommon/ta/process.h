...
namespace ta
{
    ...
    namespace Process
    {
        ...
        */
        ...
        @param[out] anStdOut Standard output
        @param[out] anStdErr Standard error
        @param[in] aMaxExecTtlMsec time in msec to wait for the command to start. The command might exit before the timeout reached or fail to start e.g. because command file is not found.
        @return true if the command has been finished or not started within aMaxExecTtlMsec interval In this case anExitCode, anStdOut and anStdErr are respectively filled with exit code, stdout and stderr output of the exited command. It is then the responsibility of the caller to interpret these values correctly.
                Otherwise we consider the command is still running detached and the function return false

         @throw ProcessExecError
        */
        static const unsigned int DefAsyncExecTtlMsec = 2000;
        bool shellExecAsync(const std::string& aCommand, std::string& anStdOut, std::string& anStdErr, unsigned int& anExitCode, const unsigned int aMaxExecTtlMsec = DefAsyncExecTtlMsec);
        bool shellExecAsync(const std::string& aCommand, std::vector<unsigned char>& anStdOut, std::vector<unsigned char>& anStdErr, unsigned int& anExitCode, const unsigned int aMaxExecTtlMsec = DefAsyncExecTtlMsec);
        bool shellExecAsync(const std::string& aCommand, unsigned int& anExitCode, const unsigned int aMaxExecTtlMsec = DefAsyncExecTtlMsec);
